...
a = 0.5 + 0.05*randn(N,M);
b = 0.5 + 0.05*randn(N,M);
c = 0.5 + 0.05*randn(N,M);
...
axis([0.47 0.53 0.0 60.0]);
xd=0.47:0.001:0.53; g=normpdf(xd,avg(1),sigma(1));
line(xd,g,'Linewidth',2,'Color','r')
subplot(1,3,2)
[h,c]=hist(lse(2,:),nbin);  
xc=c(2)-c(1);  h=h/(M*xc);  bar(c,h)
axis([0.3 0.7 0.0 12.0]); 
xd=0.3:0.005:0.7; g=normpdf(xd,avg(2),sigma(2));
line(xd,g,'Linewidth',2,'Color','r') 
subplot(1,3,3)
[h,c]=hist(lse(3,:),nbin);
xc=c(2)-c(1);  h=h/(M*xc);  bar(c,h)
axis([0.3 0.7 0.0 12.0]);  
xd=0.3:0.005:0.7; g=normpdf(xd,avg(3),sigma(3));
line(xd,g,'Linewidth',2,'Color','r')  
